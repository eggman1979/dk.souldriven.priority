...
public class EditAndSaveListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (isEditMode) {
            ...
            try {
                if (closeString != null && closeString.equalsIgnoreCase("Closed") && !entry.getIsClosed()) {
                    System.out.println("evaluating close");
                    entry.setIsClosed(true);
                    mvc.getPriorityList().moveTodoToDone(entry);
                }
                if (closeString != null && closeString.equalsIgnoreCase("open") && mvc.getPriorityList().getDoneList().contains(entry)) {
